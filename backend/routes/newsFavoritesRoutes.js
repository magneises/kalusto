...
router.get("/:userId", async (req, res) => {
  ...
  try {
    ...
    // Optional improvement: return an empty list instead of 404
    if (!favorites) {
...
router.delete("/:userId", async (req, res) => {
  ...
  if (!articleId && !articleTitle && !articleUrl) {
    return res.status(400).json({
      message: "Please provide articleId, articleTitle, or articleUrl to delete."
    });
  }
  ...
  try {
    ...
    favorites.newsArticles = favorites.newsArticles.filter((article) => {
      if (articleId) return article._id.toString() !== articleId;
      if (articleTitle) return article.articleTitle !== articleTitle;
      if (articleUrl) return article.articleUrl !== articleUrl;
      return true;
    });
